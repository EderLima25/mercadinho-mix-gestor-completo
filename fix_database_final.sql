-- Script final para corrigir o banco de dados
-- Execute este script no SQL Editor do Supabase

-- 1. Primeiro, vamos verificar e corrigir a tabela profiles
...
-- 2. Verificar e criar tabela user_roles se não existir
...
-- 3. Remover e recriar apenas as tabelas novas (suppliers, cash_registers, cash_movements)
...
-- 5. Criar tabela de controle de caixa
...
-- 6. Criar tabela de movimentações de caixa
...
-- 7. Adicionar colunas às tabelas existentes (se não existirem)
...
-- 9. Criar políticas simples (permissivas para desenvolvimento)
-- User roles
...
-- 10. Atualizar perfis existentes que não têm full_name
...
-- 11. Criar perfis para usuários que não têm perfil
INSERT INTO public.profiles (id, full_name, created_at)
SELECT 
  au.id, 
  COALESCE(au.raw_user_meta_data->>'full_name', au.email, 'Usuário') as full_name,
  au.created_at
FROM auth.users au
WHERE au.id NOT IN (SELECT id FROM public.profiles)
ON CONFLICT (id) DO NOTHING;

-- 12. Criar roles para usuários que não têm role
...
-- 14. Criar função para novos usuários (se não existir)
...
-- 17. Contar registros nas tabelas
SELECT 
  'profiles' as tabela, COUNT(*) as registros FROM public.profiles
UNION ALL
SELECT 
  'user_roles' as tabela, COUNT(*) as registros FROM public.user_roles
UNION ALL
SELECT 
  'suppliers' as tabela, COUNT(*) as registros FROM public.suppliers
UNION ALL
SELECT 
  'cash_registers' as tabela, COUNT(*) as registros FROM public.cash_registers
UNION ALL
SELECT 
  'cash_movements' as tabela, COUNT(*) as registros FROM public.cash_movements;

-- Finalizar
SELECT 'Banco de dados corrigido e configurado com sucesso!' as resultado;